...
def patch_build_script(repo_root):
    ...
    insert_block = (
        ...
        '             # Enable Man page generation in Doxyfile\n'
        '             sed -i "s/^GENERATE_MAN.*/GENERATE_MAN = YES/" doc/Doxyfile\n'
        '             make apidoc\n'
        '             \n'
        '             # Prepare output directories\n'
        '             mkdir -p "\$DOC_DIR/api/html_txt" "\$DOC_DIR/api/html_md"\n'
        '             mkdir -p "\$DOC_DIR/api/man_txt" "\$DOC_DIR/api/man_md"\n'
        '             \n'
        '             # Copy HTML documentation\n'
        '             if [ -d "doc/doxy/html" ]; then\n'
        '                 mkdir -p "\$DOC_DIR/api/html"\n'
        '                 cp -r doc/doxy/html/* "\$DOC_DIR/api/html"\n'
        '             else\n'
        '                 echo "HTML API documentation generation failed or output not found."\n'
        '             fi\n'
        '        fi\n'
        ...
        '        fi\n'
        '        if command -v pandoc >/dev/null 2>&1; then\n'
        '            echo "Starting HTML/Man to Markdown/Text conversion..."\n'
        '            # 1. Convert from HTML (Complete Coverage)\n'
        '            find "\$DOC_DIR/api/html" -type f -name "*.html" -print0 | while IFS= read -r -d "" html; do\n'
        '                base="\$(basename "\$html" .html)"\n'
        '                echo "Converting HTML \$base.html -> \$base.md / \$base.txt"\n'
        '                pandoc -f html -t markdown "\$html" -o "\$DOC_DIR/api/html_md/\$base.md"\n'
        '                pandoc -f html -t plain "\$html" -o "\$DOC_DIR/api/html_txt/\$base.txt"\n'
        '            done\n'
        '            \n'
        '            # 2. Convert from Man pages (High Quality)\n'
        '            if [ -d "doc/doxy/man/man3" ]; then\n'
        '                for man in doc/doxy/man/man3/*.3; do\n'
        '                    [ -f "\$man" ] || continue\n'
        '                    base="\$(basename "\$man" .3)"\n'
        '                    echo "Converting Man \$base.3 -> \$base.md / \$base.txt"\n'
        '                    pandoc -f man -t markdown "\$man" -o "\$DOC_DIR/api/man_md/\$base.md"\n'
        '                    pandoc -f man -t plain "\$man" -o "\$DOC_DIR/api/man_txt/\$base.txt"\n'
        '                done\n'
        '            else\n'
        '                echo "Man pages not found in doc/doxy/man/man3"\n'
        '            fi\n'
        '            \n'
        '            # 3. Convert User Docs (HTML to MD/TXT)\n'
        '            # Note: We skip API docs here as they are handled above\n'
        '            find "\$DOC_DIR" -maxdepth 1 -type f -name "*.html" -print0 | while IFS= read -r -d "" html; do\n'
        '                base="\$(basename "\$html" .html)"\n'
        '                dir="\$(dirname "\$html")"\n'
        '                echo "Converting User Doc \$base.html -> \$base.md / \$base.txt"\n'
